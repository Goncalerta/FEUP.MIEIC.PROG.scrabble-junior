...
void promptWidth(Board &board) {
    ...
    while(true) {
        ...
        else cout << "The given input is invalid.";
    }
}
...
int main() {
    ...
    do {
        ...
        Word word(command);
        if(!word.inDict()) {
            cout << "The given word wasn't found in the dictionary.";
            continue;
        }

        if(!board.addWord(word)) {
            cout << "The given word doesn't fit the board or collides with incompatible words at given position.";
            continue;
        }
    } while(!cin.eof());
